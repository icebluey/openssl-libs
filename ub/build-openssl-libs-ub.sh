...
_build_zstd() {
    ...
    install -m 0755 -d "${_private_dir}"
    cp -af usr/lib/x86_64-linux-gnu/*.so* "${_private_dir}"/
    rm -f /usr/lib/x86_64-linux-gnu/libzstd.*
    sleep 2
    /bin/cp -afr * /
    sleep 2
    cd /tmp
...
_dist=ub$(cat /etc/os-release | grep -i 'VERSION_ID' | awk -F= '{print $2}' | sed 's|[".]||g')
...
echo
sleep 1
tar -Jcvf _output/openssl-libs_${_openssl_ver}-${_glibc_ver}_${_dist}_amd64.tar.xz openssl-libs
echo
sleep 1
/bin/rm -fr /usr/local/private
/bin/rm -fr /tmp/openssl-libs
